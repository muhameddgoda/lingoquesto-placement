  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <Badge className={`${getQuestionTypeColor(question.q_type)} flex items-center space-x-2`}>
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {question.metadata?.imageRef && (
            <div className="mb-4">
              <img 
                src={`/images/${question.metadata.imageRef}`} 
                alt="Question image"
           ...
           question.metadata.context.question !== question.prompt && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-blue-800 text-sm">
                <strong>Additional context:</strong> {question.metadata.context.question}
              </p>
            </div>
          )}

          {/* Expected Response Time (for speaking questions) */}
          {(question.q_type === 'open_response' || question.q_type === 'image_description') && 
           question.metadata && (
            <div className="mt-4 flex items-center justify-center space-x-6 text-sm text-gray-600">
              {question.metadata.think_time && (
                <div className="flex items-center space-x-1">
                  <span>Think time:</span>
                  <span className="font-medium">{question.metadata.think_time}s</span>
                </div>
              )}
              {question.metadata.response_time && (
                <div className="flex items-center space-x-1">
                  <span>Response time:</span>
                  <span className="font-medium">{question.metadata.response_time}s</span>
                </div>
              )}
            </div>
          )}
